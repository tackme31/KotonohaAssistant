...
public interface ILazyModeHandler
{
    ...
    /// 怠け癖モードを処理します
    /// </summary>
    Task<(LazyModeResult result, ConversationState state)> HandleLazyModeAsync(
...
public class LazyModeHandler : ILazyModeHandler
{
    ...
    /// 怠け癖を発動すべきかどうかを判定します
    /// </summary>
    private bool ShouldBeLazy(ChatCompletion completionValue, ConversationState state)
    {
        // 関数呼び出し以外は怠けない
