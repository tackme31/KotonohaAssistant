...
/// <summary>
/// テスト用のモック IAlarmClient
/// </summary>
public class MockAlarmClient : IAlarmClient
{
    public Func<Task>? AddAlarmFunc { get; set; }
    public Func<Task>? StopAlarmFunc { get; set; }
    public Func<Task>? StartTimerFunc { get; set; }
    public Func<Task>? StopTimerFunc { get; set; }

    public int AddAlarmCallCount { get; private set; }
    public int StopAlarmCallCount { get; private set; }
    public int StartTimerCallCount { get; private set; }
    public int StopTimerCallCount { get; private set; }

    public Task AddAlarm(TimeSpan time, string voicePath, bool isRepeated)
    {
        AddAlarmCallCount++;
        return AddAlarmFunc?.Invoke() ?? Task.CompletedTask;
    }

    public Task StopAlarm()
    {
        StopAlarmCallCount++;
        return StopAlarmFunc?.Invoke() ?? Task.CompletedTask;
    }

    public Task StartTimer(TimeSpan time)
    {
        StartTimerCallCount++;
        return StartTimerFunc?.Invoke() ?? Task.CompletedTask;
    }

    public Task StopTimer()
    {
        StopTimerCallCount++;
        return StopTimerFunc?.Invoke() ?? Task.CompletedTask;
    }
